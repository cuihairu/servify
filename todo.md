...
- 附注（2026-09-26，**Phase 2 刀二c Web 语音上行 + 字幕渲染已落地**——core MicCapture + vanilla UMD 挂载 + demo 挂件端到端形态）：
  ...
  - 阻塞项：无
  - 完成证据：core `npx vitest run` 125/125 全绿（新增 25：纯函数数值面 + 接线分支 + stop 竞态两用例）+ `tsc --noEmit` 干净；vanilla test/typecheck 绿、`vite build` 过（MIXED_EXPORTS 警告系既有 named+default 并存提示非错误）+ UMD 冒烟（模拟 window 验证 `Servify.VoiceChannel/MicCapture` 等六件挂载 + `pcm16ToLeBytes(floatToPcm16([1,-1]))` → `[255,127,1,128]` 往返正确）+ `node --check widget.js` 过；demo-sdk 三产物已重同步；Android/iOS 零改动。
