...
- 附注（2026-09-26，**Phase 2 刀二c Web 语音上行 + 字幕渲染已落地**——core MicCapture + vanilla UMD 挂载 + demo 挂件端到端形态）：
  - 状态：`[-]`（刀二 provider 面 + b-1 管线 + b-2 服务端通道 + b-3 协议三件套 + b-c Web 侧消费已落地；剩余 = Android/iOS 语音通道客户端与上行/字幕渲染，按产品节奏；管理端坐席侧语音入口未排期）
...
- 附注（2026-09-26，**Phase 2 刀二b-3 协议三件套已落地**——PROTOCOL §9 + fixtures + 三端回放）：
  - 状态：`[-]`（刀二 provider 面 + b-1 管线 + b-2 服务端通道 + b-3 协议三件套已落地；下一步刀二c = 三端语音上行/字幕渲染消费）
