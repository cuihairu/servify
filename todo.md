...
- 当前优先恢复任务：按用户指示推进下一项。P2-6 前六刀完成（第一刀会话转接 §7 七项、第二刀满意度 §8 九项、第三刀客服/客户管理 §4+§5 五项、第四刀统计/排班 §10 六项、第五刀宏/集成/自定义字段 §9 三项、第六刀远程协助+辅助建议（§2 新增远程协助行 + §11 辅助建议行，`make remote-assist-acceptance` 入库，含 WS 真实访客会话/协助发起列表详情/标注增删查升序对账/带录制结束/未认证与 GET-POST 相似工单对账 20 项检查），均从未验转通过，`make session-transfer-acceptance` / `make satisfaction-acceptance` / `make customer-agent-acceptance` / `make statistics-acceptance` / `make macro-integration-customfield-acceptance` / `make remote-assist-acceptance` 入库）；剩余未验分散项：自动化三行（§10B）、激励排行、pgvector 自建知识库（需 pg）；下一步候选 P2-7 SDK 与多端 contract 稳定性治理或清剩余散项；另:第六刀验收发现远程协助删除不存在标注返回 500（repo 裸错误未映射 ErrAssistNotFound，checklist 已注明）待后续单独修
