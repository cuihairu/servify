...
- 分刀计划（沿 P1-7 七步模板，每刀独立提交）：
  ...
  - 第三梯队（新 module 真实迁移，小到大）：api_key→macro→custom_field→workspace→shift→app_integration→auth（✅ 刀13）→satisfaction（✅ 刀14）→sla（需先解 ticket orchestration 对 legacy 的窄接口依赖）；realtime 三件按"运行态硬化"持续收窄不套模板
