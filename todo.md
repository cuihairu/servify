...
### [x] P0-1 事件总线仍默认使用进程内 `InMemoryBus`
...
### [x] P0-2 Voice 运行时仍依赖 mock provider
...
### [x] P0-3 Agent 运行态仍依赖内存注册表与 legacy runtime 适配层
...
### [x] P0-4 配置加载仍存在直接 `panic`，且默认模型配置偏旧
...
### [x] P0-5 Fallback 配置仍暴露 `legacy_kb_enabled`，兼容语义未完全收口
...
### [x] P2-0 客户侧推荐问题与上下文联想问题（含 RQ-5 曝光/转化归因）
...
- 建议拆分：
  ...
  - `RQ-5` 埋点、点击率、转化率与验收口径 ✅ 曝光/转化归因全链落地（服务端口径，见下）；点击率未单独埋点——demo widget 点击即发送，"看了推荐并提问"由转化口径覆盖，独立 click 事件留产品需要时再加
- 验收标准：
...
- 状态：`[x]`（2026-09-22 收官：核心链路 + RQ-5 曝光/转化归因 + 验收口径全链落地）
- 最近进展：**2026-09-22 RQ-5 收官**（切片提交 3b8370e，CI 35688179671 绿；服务端归因口径定为「曝光 = initial/next 接口成功返回即落一行（`suggestion_exposure_logs`，questions 存 JSON 数组）；转化 = 同 session 后续客户消息与最近一次未转化曝光的问题文案规范化匹配（空白折叠+小写），命中即回填 converted_question/converted_at，并发双命中以 `converted_question` 空串守卫保证先到先得；session_id 为空仅计曝光不入归因」。实现：application 层 `recordExposure`/`NormalizeQuestionText`/`MatchSuggestionConversion` + contract 透传 `session_id` + infra 四方法（RecordExposure/FindLatestOpenExposure/MarkExposureConverted/ExposureSummary）+ 迁移 000010；**WS 接线**：realtime 窄接口 `suggestionConversionRuntime`（签名与 Service 完全一致零适配器）+ `SetSuggestionConversionService` + `handleTextMessage` 持久化后同步匹配（失败仅 Warn 不影响消息主流程）+ 内容提取收敛 `textMessageContent` 助手 + runtime_assembly 单实例共享（suggestion module 贯穿 REST 管理面与 WS 归因）；**SDK 消费**：core `getInitialQuestions(limit, {sessionId})` + demo widget `fetchSuggestQuestions` 统一带 session_id（无 session 不带参仅计匿名曝光）+ demo-sdk bundle 同步；**验收口径**：`test-suggestion-acceptance.sh` 第 9 步（真实模式）——initial/next 带 session_id 产出曝光行 → node 原生 WebSocket 发送匹配客户消息 → 直读 sqlite 断言 kinds=[initial,next] 且 converted=[匹配文案]，manifest 增 `exposure_conversion_ok` 键（mock/外部模式 skipped），mock 模式 Go 测试契约同步。全量门禁本地绿（gofmt/vet/boundaries/race/100% 逐包覆盖率/SDK vitest+typecheck/验收脚本 mock+real 双模式），CI 终态见完成证据。既有 38566cf（曝光/转化仓储与应用层）先行合入且 CI 绿）。同日追加固化刀（用户确认两项产品决策后执行）：ExposureSummary 管理面出口 `GET /api/assist/suggestions/exposure-summary`（assist 权限组，contract/delivery/handler 全链 + 应用层透传 + 适配器映射测试 + 验收脚本管理面聚合对账 total=5/converted=1/next=1）+ 三模板 `/public/suggestions/` 独立路径级限流（staging 45rpm/burst10、生产与 dev 30rpm/burst10）**并修正专用前缀被 `/public/` 首匹配遮蔽的顺序缺陷（kb/csat 专用限流此前为死配置，一并复位）** + `public-surface-security-checklist.md` 补 suggestions 段）
- 下一步：无
- 阻塞项：无
- 完成证据：
  - 代码文件：`apps/server/internal/modules/suggestion/application/customer_questions.go`、`apps/server/internal/modules/suggestion/infra/gorm_repository.go`、`apps/server/internal/platform/realtime/websocket_hub.go`（suggestionConversionRuntime + SetSuggestionConversionService + textMessageContent）、`apps/server/internal/app/server/runtime_assembly.go`（单实例共享装配）、`sdk/packages/core/src/api.ts` + `sdk.ts`、`apps/demo-sdk/widget.js`、`scripts/test-suggestion-acceptance.sh`
  - 测试命令：`go test -race -count=1 ./internal/platform/realtime ./internal/app/server ./internal/modules/suggestion/...`、`npm -C sdk run test`、`bash scripts/run-tests.sh`（100% 逐包）、`bash scripts/test-suggestion-acceptance.sh`（real 模式含第 9 步曝光-转化断言 + mock 模式 Go 契约）
  - 文档或验收回填位置：本文条目 + `scripts/test-results/suggestion-acceptance/manifest.json`（exposure_conversion_ok）
...
- 进展（分刀推进）：
  ...
  - 第八刀（assist 模块死链卫生扫描，2026-09-20）✅
    - 刀 24 方法论首次对单模块全面应用：modules/assist + handlers 两个
      assist handler 共 41 个导出符号逐个生产消费三分类（生产消费/测试
      回读/注释提及）
    - 唯一死链：`Repository.GetAnnotation`（标注单条读取）——接口声明 +
      GORM 实现 + RoundTrip 测试回读（命中/未命中/掉表三分支）+ 两个
      mock 被迫实现，生产零调用（标注读取只走 ListAnnotations 列表端点）
    - 清理：删接口方法 + 实现 + 两个 mock 被迫实现；RoundTrip 改用
      ListAnnotations 读回对账（保留 Create 落库验证，换消费方法）；
      not found 语义链无损失（service_test.go 真实 DB 已覆盖
      DeleteAnnotation 的 RowsAffected==0 → ErrAssistAnnotationNotFound）
    - 确认存活：装配链 6 入口、9 接口方法、8 个 HandlerService 方法、
      7 个错误常量（assistErrorStatus 映射）、命令/请求类型全部有真实
      消费；Service.AttachRecording 是编排方法非死代码
    - 门禁：构建绿、assist 三包 100% 保持、gofmt 绿、全量门禁实测
      100.0%；净 -35 行；提交 ac9ef2c，CI 35489519876 绿
  - 第九刀（全仓死代码普查——A 类收口刀孤儿清除，2026-09-20）✅
    - 方法：`golang.org/x/tools/cmd/deadcode@v0.30.0` 四入口（server/
      dbrecovery/gen-baseline/migrate）可达性分析取交集 + grep 三分类 +
      人工复核 131 条候选（通用方法名 grep 有词法污染，人工复核必做）
    - A 类已清（提交 88d633a，净 -1107 行）：「收口刀孤儿」——rules.go
      等注释自证「centralizes ... so legacy runtime facades do not
      drift」，为 legacy 门面写的集中语义函数在刀 18 删 services 后成
      孤儿（routing 7+1+2、ticket 1、ai 2、agent 1、automation 导出包装
  ...
  - 独立遗留项（2026-09-20 登记，产品口径）：知识库选型决策——用户确认
    ...
    ——已于 2026-09-21 拍板执行并落地（用户决策：ragflow 最高优先 /
    租户作用域全量对齐 dify/weknora / 版本锚定 v0.27.x，完成记录见
    恢复点附注 2026-09-21 条）
...
- 附注（2026-09-26，**Phase 2 刀二d 成本控制收口已落地**——逐句预算熔断 + 业务计量，§3.2 控制手段收口）：
  - 状态：`[-]`（刀二 provider 面 + b-1 管线 + b-2 服务端通道 + b-3 协议三件套 + b-c Web 侧消费 + d 成本控制已落地；剩余 = Android/iOS 语音通道客户端与上行/字幕渲染，按产品节奏；管理端坐席侧语音入口未排期；Phase 3 租户配额限流仍待 RA-7 后盘）
  - 最近进展：设计文档 §3.2 两项已设计未实现的控制手段落地。**逐句预算熔断**——`application/voice.go` 管线对翻译/TTS 各加单句超时（`sentenceTranslateBudget`/`sentenceSynthBudget` = 5s，构造默认、字段可注入测试；依据 §2.1 P50 600ms/800ms 给裕量、远小于 provider 传输层 30s 兜底——挂死上游句降级而非把串行管线拖 30s）；超限走既有降级面（翻译超时 → `errors.Is(DeadlineExceeded)` 降级原文、合成超时 → 仅字幕），管线不断；**收线守卫**——会话拆除（外层 ctx cancel）中的在途句不再产出降级字幕/错误帧（那是收线不是熔断，此前会在 teardown 时冒出幽灵字幕）。**业务计量**——`VoiceObserver` 观测面（`OnSentenceOutcome` outcome 词表 `VoiceOutcomeTranslated`/`VoiceOutcomeCaptionOnly`/`VoiceOutcomeDegraded` + `OnTranslateTokens` 逐句 token 消费；nil 安全，管线串行调用实现须非阻塞）；delivery 侧 `voiceMetricsObserver` 桥接 `rt.BusinessMetrics`（新计数器 `voice_translation_sentences_total{outcome}`，telemetry 常量 `MetricVoiceTransSentences`；token 复用既有 `ai_llm_tokens_total{provider,token_type}` input/output 口径，对齐 enhanced_service 的字面量口径）；`TranslateResult` 增可选计量字段 `Provider`/`TokenUsage *llm.TokenUsage`（omitempty——REST 响应形状 additive 变化，openai/anthropic provider 早已回传这两字段只是被丢弃，PROTOCOL §5 已注"消费方可忽略"）；装配 `runtime_assembly.go` 传 `rt.BusinessMetrics` 进 `NewVoiceChannelService`（nil 允许 = 不计量）。观测意义：hub 的 `voiceFrameSink.OnError` 只记服务端日志（失败零帧原则），句子级降级此前完全不可观测——outcome 计数器是唯一面。
  - 已知边界：预算 5s 是常量非配置面（租户级配置走 Phase 3 配额刀一并盘）；`caption_only` 无法区分"synth 未配置"与"synth 失败"（部署形态看装配日志，句级只看比例）；token 计量依赖 provider 回传 usage（不回传则静默无数据，与 REST 面同口径）。
  - 下一步：Android/iOS 语音上行 + 字幕渲染消费（按产品节奏）；或管理端坐席侧语音入口；Phase 3（WebRTC 音轨下发 + 租户配额限流）之前服务端无新广播点。
  - 阻塞项：无
  - 完成证据：translation/application + delivery + observability/metrics 单测全绿（新增：预算熔断两用例 + 收线守卫 + observer 四 outcome/token 断言 + 适配器计量端到端 + `RecordVoiceTranslationSentence` 计数/nil 两用例；预算用毫秒级注入不拖慢套件）；`go vet ./apps/server/...` 过；全量门禁见下条提交前记录。
- 附注（2026-09-26，**Phase 2 刀二c Web 语音上行 + 字幕渲染已落地**——core MicCapture + vanilla UMD 挂载 + demo 挂件端到端形态）：
  - 状态：`[x]`（刀二全阶段已落地：provider 面 + b-1 管线 + b-2 服务端通道 + b-3 协议三件套 + b-c Web 侧消费；剩余 = Android/iOS 语音通道客户端与上行/字幕渲染，按产品节奏；管理端坐席侧语音入口未排期）
  - 最近进展：core `voice-capture.ts`（`sdk/packages/core/src/voice-capture.ts`）——纯函数三件（`floatToPcm16` clamp 量化含 NaN 归 -1 / `resampleLinear` int16 域线性插值、非正采样率 RangeError、同率/空输入原样返回 / `pcm16ToLeBytes` DataView 显式小端平台序无关）+ `MicCapture` 采集图（getUserMedia → MediaStreamSource → ScriptProcessor(4096,1,1) → 零增益 GainNode → destination——ScriptProcessor 必须接入渲染图才有回调，零增益避免回声；选型 ScriptProcessor 而非 AudioWorklet 的原因：processor 模块经 Blob URL 加载会被服务端 `default-src 'self'` CSP 拦截，ScriptProcessor 无模块加载全浏览器可用、代价仅主线程回调且各厂商无移除时间表；generation 作废在途 start——stop 在授权弹窗停留期间到达时，授权回来只停刚拿到的轨绝不接线；`capture_denied`（NotAllowedError/SecurityError，不可重试）/`capture_unavailable`（可重试）/`capture_already_active` 错误归一；ports 注入（getUserMedia/audioContextFactory）对齐 webSocketFactory/peerConnectionFactory 口径）+ `ServifyErrorCode` 新增三枚采集错误码 + core index 导出 + vanilla UMD 全局挂载（`window.Servify.MicCapture/VoiceChannel/VOICE_SAMPLE_RATE/floatToPcm16/resampleLinear/pcm16ToLeBytes`——UMD 包装器的 exports 全局会被模块体内 `window.Servify = 类` 赋值覆盖、named 导出在浏览器不可达，语音工具集用 `Object.assign` 挂到类上，`ServifyGlobal` 类型显式声明）+ demo 挂件（`apps/demo-sdk/widget.js`，经并发会话协作两轮合成，见下条）：面板头部麦克风按钮（能力探测：旧缓存包没有两个类时不出现、聊天主链路零影响；live 红点 + micOn/micOff 图标切换）+ **会话 id 一处生成多通道共用**（原实现 WSClient 自造 `ws_<ts>` 而外层 `sessionId` 变量留空，语音通道与补拉/推荐归因拿不到同一 id——行为变化：自动 id 会话的断线补拉从跳过变为生效，公开路由 404 静默兜底）+ `voice:delta` 直播行（每说话方一条、新轮次顶旧轮次、▌ 光标）+ `voice:final` 终句字幕（译文 + 原文小字 + 未翻译 chip 标注）+ `voice:audio` 播报（`data:audio/*;base64` 自动播放，被浏览器自动播放策略拦截退化为 ▶ 按钮）+ `voice-error` 系统提示收线 + 会话通道断线/面板关闭联动语音收线。
  - 并发协作与审计（opencode 同仓在跑，Request 4 口径）：本刀期间它提交了 4b3ea04（修正我在 voice-capture.test.ts 的两处断言错误——`Math.round(-16383.5)` 因 JS .5 朝 +∞ 取整为 -16383 而非 -16384；ratio=2 重采样取偶数下标故 input[1] 不出针——两处均验算属实）与 f1737d1（vanilla 导出），并一度用其自版 widget 语音实现整文件覆盖我的版本；审计其自版发现五处缺陷并修复：①单字幕气泡跨说话方互串（双方 delta 改写同一气泡且角色锚定首个说话方）→ 每说话方独立直播行 + seq 键字幕；②WebAudio 播放无 suspended 处理、自动播放拦截即静默失败 → Audio 元素 + play().catch 退化 ▶ 按钮；③麦克风回调对断连静默丢帧（capture 挂死空转）→ sendAudio try/catch 收线 + 提示；④`/api/v1/guest/session` 是 service API key 面（宿主后端调用），浏览器 fetch 必 401，属死代码 → 删除，改 `opts.accessToken` 注入；⑤麦克风按钮未按 SDK 能力隐藏 → 能力探测门控。另收敛其 100ms 轮询 + 10s 超时为 connected 事件驱动，去掉硬编码 `debug: true`。
  - 已知边界：移动端（Android `AudioRecord` / iOS `AVAudioEngine`）语音通道客户端与上行/字幕渲染未落（PROTOCOL §9.2 端级偏差声明维持）；demo 挂件是演示壳无自动化测试面（核心数学与接线分支在 core 125 用例内覆盖；挂件只有 `node --check` + UMD 冒烟）——生产化字幕 UI 需另立刀；自动播放策略各浏览器差异大，拦截退化按钮是当前兜底；ScriptProcessor 已弃用但无移除时间表，AudioWorklet 升级是采集图半边直换（需先解决 CSP 或宿主同源托管 processor 模块）。
  - 下一步：Android/iOS 语音上行 + 字幕渲染消费（按产品节奏）；或管理端坐席侧语音入口；Phase 3（WebRTC 音轨下发）之前服务端无新广播点。
  - 阻塞项：无
  - 完成证据：core `npx vitest run` 125/125 全绿（新增 25：纯函数数值面 + 接线分支 + stop 竞态两用例）+ `tsc --noEmit` 干净；sdk 工作区 `npm test`（160 用例）/`typecheck`/`lint`（0 错误）/`test:surfaces`/`test:governance`/`test:examples` 全绿；vanilla `vite build` 过（MIXED_EXPORTS 警告系既有 named+default 并存提示非错误）+ UMD 冒烟（模拟 window 验证 `Servify.VoiceChannel/MicCapture` 等六件挂载 + `pcm16ToLeBytes(floatToPcm16([1,-1]))` → `[255,127,1,128]` 往返正确）+ `node --check widget.js` 过；demo-sdk 三产物已自含竞态修复的构建重同步 + `verify-generated-assets.sh` 过；Android/iOS 零改动；Go 面零改动仍跑全量门禁——`run-tests.sh` exit 0（覆盖 100.0%）+ build/build-weknora/race/smoke/golden/hygiene/encoding/boundaries/vet/gofmt 全绿 + security-baseline（secure 示例配置注入 env）过 + check-acceptance-evidence 过。
- 附注（2026-09-26，**Phase 2 刀二b-3 协议三件套已落地**——PROTOCOL §9 + fixtures + 三端回放）：
  - 状态：`[x]`（刀二 provider 面 + b-1 管线 + b-2 服务端通道 + b-3 协议三件套 + b-c 已完成）
  - 最近进展：`sdk/PROTOCOL.md` §9 语音通道契约（握手参数与 400/401/503 语义、上行 pcm16 24kHz 单声道 64KB 帧上限、下行四帧载荷表：`translation-delta`（speaker/turn_seq/text）、`translation-final`（speaker/seq/original/content/source_lang/target_lang/degraded）、`translation-audio`（speaker/seq/format/audio=base64 内联）、`voice-error`（code ∈ disabled/asr_unavailable/stream_broken，连接级语义只回发起连接且帧后必随 close）+ `turn_seq`（ASR 轮次）/`seq`（字幕句号，跨轮单调从 1 起）语义辨析 + 慢客户端/无历史重发/保活与会话 WS 同口径声明）+ §8 必备样例集补充语音帧族 + `sdk/protocol-fixtures/` 12-15 四样例（`channel: "voice"` 标注，core/mobile 端级偏差在 assert 内显式声明）+ core `VoiceChannel`（`sdk/packages/core/src/voice.ts`：握手参数拼装（session_id/speaker/access_token）、下行四帧必需字段校验 → `voice:delta/final/audio/error` 事件（载荷保持线序 snake_case，畸形静默忽略）、`sendAudio` 二进制上行（Uint8Array 视图拷贝）、无自动重连（语音流是活体采集会话，重启说话是用户显式动作））+ fixtures 回放断言（KNOWN_KINDS 词表 +4、四帧全载荷事件 + 会话通道管理器对误入语音帧静默忽略的纵深断言）+ Android/iOS 回放同步（词表 +4、`voiceFrameFamilyIsIgnoredByConversationCore`：会话核心按 `UnknownIgnored` 断言——两通道零共享，端级偏差声明）。
  - 已知边界：三端均无麦克风采集与字幕/TTS 播放 UI（刀二c：Web `getUserMedia`+重采样 → Android `AudioRecord` → iOS `AVAudioEngine`，payload 形态按 §9 已冻结）；移动端语音通道客户端未落（会话核心对语音帧的忽略断言是当前端级契约）；`degraded=true` 语义（降级原文、无对应 audio 帧）仅 core 类型层承载。
  - 下一步：刀二c——三端语音上行/字幕渲染消费（demo 面可先落 Web：`getUserMedia` AudioWorklet 重采样 pcm16 24kHz → `VoiceChannel.sendAudio` + `voice:final`/`voice:audio` 字幕气泡与播放；移动端按产品节奏）；或先落管理端坐席侧语音入口。
  - 阻塞项：无
  - 完成证据：core `npx vitest run` 100/100 全绿 + sdk 工作区 `npm test`/`typecheck`/`build`/`lint`/`test:surfaces`/`test:governance`/`test:examples` 全绿；服务端零改动（b-2 已就位的广播点是本刀的唯一服务端依赖），run-tests 100.0%/build/build-weknora/race/smoke/golden/hygiene/encoding/boundaries/acceptance-evidence/security-baseline（secure 配置注入 env）/vet/gofmt 本机全绿；demo-sdk 同步与 swag 重生成零漂移（`verify-generated-assets.sh` 过）；Android/iOS 用例与三端既有镜像同构（本机无 Android SDK 与 swift 工具链，单测由 CI android/ios-swift job 持续执行——M1/M2 既有口径），Kotlin 侧仅用文件内既有 helper（fixtureByKind/framesOf/replay/jsonPrimitive/UnknownIgnored）。
- 附注（2026-09-26，**Phase 2 刀二b-2 服务端语音通道已落地**——独立 WS 通道 + 装配接线）：
  - 状态：`[-]`（刀二 provider 面 + 刀二b-1 管线半边 + 刀二b-2 服务端通道已落地；b-3 协议三件套已随上一条恢复点落地）
  - 最近进展：新增独立语音通道 `/api/v1/ws/voice`（设计文档 §1.2：持续二进制音频上行不挤会话 WS 的 256 帧下行缓冲，会话 WS 契约零变更）：`app/server/security_surface.go` 登记免认证建连面 `public-realtime-voice`（`RouteSecurityWarnings` 不会对已装配 ai.asr 的部署持续告警；路径级限流沿用 `/api/v1/ws` 前缀条目，前缀匹配已覆盖本路径）+ `modules/translation/delivery/voice_contract.go`（`VoiceStreamStarter`/`VoiceAudioStream` 契约 + `VoiceStreamSink`/`VoiceCaption` app 类型别名桥接，平台层只依赖 delivery）+ `voice_adapter.go`（每条上行流组装"ASR 会话 + 语音管线"：说话方→读向对偶解析（visitor 说话查 agent 读向、agent 说话查 visitor 读向）、无偏好 = `ErrVoiceChannelInactive` 不激活、`Close` 先 cancel 管线再收 ASR 会话、源语言留空走模型自动检测、固定 pcm16/24kHz/单声道上行）+ `platform/realtime/voice_hub.go`（握手校验 session_id/speaker/token 同款 + 未配置 503、按会话广播、慢客户端踢线与会话 WS 同口径、voice-error 收线且错误帧先于 close 帧到达、失败零帧只记日志；并发不变量：Send 的投递与关闭只在 Run 事件循环——收线走单一 teardown 命令通道（携带收线前最后一帧），客户端泵直投 Send 会与踢线的 close 形成 send on closed channel 进程级 panic）+ 装配接线（`ai.asr` 未配置 = 路由不注册 + hub 握手 503 兜底；guest token 校验器与会话 WS 同源实例；`Runtime.Start` 随 `Run()` 起 hub 事件循环）。
  ...
  - 已知边界：客户端尚无消费（协议三件套是刀二b-3，按 PROTOCOL §5 流程服务端广播点已就位、禁止客户端单侧预留）；音频载荷形态已定 base64 内联（每句 mp3 单帧，<100KB 级），若后续双工大音频再议短时 URL；ASR/TTS provider 未配置时该路由在装配层不存在（401/503 之外无半装配形态）。
  ...
  - 阻塞项：无
  - 完成证据：`platform/realtime` + translation 四包 + `app/server` 逐函数 100% 覆盖、语音用例 `-race -count=3` 绿、vet/gofmt 绿；run-tests/build/golden/smoke/hygiene/encoding/boundaries/security-baseline（配置项告警为 dev config 既有项）全绿。
- 附注（2026-09-26，**Phase 2 刀二b-1 管线半边已落地**——分句器 + 每说话方语音管线）：
  - 状态：`[x]`（刀二 provider 面 + 刀二b-1 管线半边已落地；刀二b-2 WS 面已落地）
...
- 附注（2026-09-22，「AI 帮忙回信息」主链路智能化四批收官）：按用户指令把该主链路做成产品亮点，按价值排序分四批交付 + 一笔门禁修复，全部提交推 main；**完成证据锚定最终树 1a7fe17 → CI run 35767697076 绿（覆盖 batch a/b + boundary 修复 + batch d 全部提交，此前 run 均被 push cancel-in-progress 取消故以本 run 为准）**：
  ...
  - **boundary 修复（18a0a25）**：module-boundaries.rules 49-50 改两条变量形态 pinned（`historyAdapter :=` 构造 + `SetConversationMessageWriter` 接线），延续「一重构一刀一规则更新」惯例；其 CI run（35763501404）被后续 push cancel。
  - **batch b 坐席 AI 辅助 Copilot（72cea90）**：`POST /api/v1/ai/copilot` 单端点按 `action` 分发三类能力——`suggest_reply`（按会话近期历史起草回复）/`rewrite`（按语气改写坐席草稿）/`session_summary`（三段式摘要），与首答共用 provider、出站参数与会话历史口径；历史类动作用「明确失败」语义（无可读消息 4xx、不静默编造上下文），provider 未装配降级 503；附带 RBAC 前置修复——workspace 资源组 POST 无权限映射恒 403，四份 config 模板补 `workspace:write` 坐席可写 + permissions_catalog 展示性元数据死代码清理；其 CI run（35762549026）被 cancel，同树验证随 batch d run 收口。
  - **batch d 首答流式输出（1a7fe17，本批 CI 绿）**：编排层 `HandleStream` 与 `Handle` 共享 prepare/finalize 流水线（策略钩子/护栏/检索/prompt/审计同链），ChatStream 增量即产即发、agent 工具循环照常（分片中完整工具调用执行后继续，最终内容为全部增量拼接；提取 `appendToolFeedback` 供两条循环复用；调用方取消 ctx 发射侧放弃）；delivery `ProcessQueryStream` 转人工关键词短路单事件直达（不碰模型）、`(nil,nil)` 流归一为错误、终末事件 finalizeEnhanced 与非流式等价（sources/strategy 带回终帧）；scoped 包装器经可选能力接口委托（`RuntimeQueryStreamer`），能力缺失显式报错；WS hub 窄接口协商流式——`ai-response-delta` 帧 `{content_delta, done}` 即产即发 + 终末 delta（`done=true`）+ 完整 `ai-response` 终帧（帧契约向后兼容），流启动即失败整体回退单发、增量已推送后失败只补终末 delta 不回退不补发（防文本重复）；mock provider `StreamQueue`/`StreamErrorQueue` 按次消费支持多步编排。21 个新测试，五个触包覆盖率均 100%、race 绿。文档同步：ARCHITECTURE.md §10 流式架构段 + docs/mobile-sdk-design.md 流式已接出口径。
  - 另同日：移动端 SDK 策划文档（8e9766f，docs/mobile-sdk-design.md——Android/iOS 嵌入式客服 SDK 设计，给接入方 App 内嵌客服能力，非自做客服 App）；**两轮深化收口**——①事实核查修订（bdf3af6）：对照服务端路由注册/广播点/中间件链逐项核实，修正四处事实错误（转人工状态机原写 session_update/agent_status 系 core 死分支幻影帧、真实帧为 transfer_notification/waiting_notification；REST 依赖 /api/v1/ai/query、/api/tickets、/api/omni/* 全部管理面访客不可达、WS 是访客唯一现成通道；WS 握手 access_token 参数服务端不消费；流式接出未承接进里程碑），补决策 D2 缺号，剩余数字类声明（重连 5 次 1s 起 2 倍封顶 30s、心跳 30s、createMobileCapabilitySet voice/remote_assist off、app-core 四契约、SessionSnapshot 形状、vitepress 站点注册两处）全部源码核实无误；②契约文档先行（sdk/PROTOCOL.md 首版，M0 头号交付物提前落地）：逐帧契约（上行 4 类/下行会话·转人工·webrtc 三族）+ core 六死分支帧单独立表 + 心跳双向机制澄清（服务端协议层 Ping 54s、core 的 JSON system/ping 帧服务端不处理，移动端口径走原生协议层保活）+ 慢客户端 256 帧踢线/无 ACK 判据=回显/无历史重发等边界语义 + kebab/snake 命名冻结 + fixtures 互验约定；③开工技术方案文档（docs/mobile-sdk-platform-spec.md，按策划里程碑"文档先行"的第三层）：两平台同构三层分层架构（L0 协议层←PROTOCOL.md 契约 / L1 headless 会话核心——一个核心服务探针·契约回放·RN·原生 UI 四消费者 / L2 Compose·SwiftUI UI 层，依赖单向 L2→L1→L0，宿主只见 ServifyChat 门面）+ Android 选型工程化（Kotlin 2.x/compileSdk 35 运行时 minSdk 24 兜底、单 AAR internal 分层、SupervisorJob+StateFlow/SharedFlow、DataStore+EncryptedSharedPreferences、consumer R8 rules、CI 只构建验证不内置自动发版）+ iOS 工程化（XCFramework+SPM binary、源码期多 target 打包合并、structured concurrency+@MainActor+AsyncStream 与 Kotlin 一一映射、Keychain+JSON 快照、PrivacyInfo.xcprivacy 不采集不追踪、ATS 明文例外口子不开）+ V1 冻结 API 表面（ServifyConfig 8 字段表含 apiUrl 强制 https 构造期报错/loggingEnabled 关闭态不打印正文与 token；门面 7 方法表含 sendMessage 成功判据=回显帧·createTicket/registerPushToken 端点未上线返回 unsupported 不静默成功；事件表对齐 core ServifyEventMap 子集并显式排除 agent_typing 死分支；连接状态机沿用 core 重连参数；错误模型 sealed class/enum 统一 code+message+retryable 七码；app-core 四契约逐一落点映射）+ 接入流程细化（最小接入 8 行级 Android/iOS 双示例、集成检查清单 5 项含体积对账、常见错误对照表——含原生 WS 无 Origin 头需服务端白名单放行空配置的服务端配置项声明）；站点注册两处 + docs build 绿。P1-1 维持原状（等外部凭证）。
  - 另同日：**M0 里程碑收官**（协议契约与联调探针，三项验收全部达成）——①契约样例集 `sdk/protocol-fixtures/` 10 例（39f2a67，PROTOCOL.md §8 schema：单帧 `frame`/多帧 `frames` + expectations 词汇表 kind 九分类/assert 关键字段断言/state 转人工合法转移对；core 9 回放测试同一套样例喂过，已知缺口显式断言：core switch 无 delta/transfer/waiting case 全落 default 忽略）；②Android 探针工程（2b96be6，`sdk/android/` 纯 JVM Kotlin 2.1.20 + kotlinx-serialization，零 Android SDK 依赖：WireFrame sealed 六族 + FrameCodec 未知/畸形降级 Unknown 不抛 + HandoffStateMachine 转移表穷举 + StreamingAssembler 三段契约拼接器（终末增量 done=true 空串 + 终帧比对拼接，迟到增量拒绝、流中断不伪造终帧）+ SessionCore consume→ProtocolEvent 词汇对齐 fixtures kind + ProbeMain CLI 全链路探针；Gradle wrapper 8.14.3 入库（本地 4.4.1 太旧下载自举），CI 新 `android-probe` job（temurin 21 + gradle cache）首跑；21/21 测试绿；Kotlin data object 泛型推断坑用显式类型常量解）；③`make mobile-probe-acceptance` 全链路联调（ff8c49b，M0 验收②）：内嵌 python mock LLM（OpenAI 兼容 /models 健康探测 + /v1/chat/completions SSE 逐 chunk 流式）+ sqlite 起服（ai.openai 指向 mock、websocket_allowed_origins 空=放行无 Origin 原生 WS）+ admin/agent 实体上线 + 探针建连→回显→AI 首答流式 4 增量→"转人工"关键词→坐席在线直转 transfer_notification→omni 注入坐席回复 agent-message→终态 AgentChatting；manifest（provider=mobile-probe mode=real）overall=passed 全 checks true 入库，validate-acceptance-manifest.sh 新增 mobile-probe 分支（10 checks 对账 + 5 证据文件必列，照 per-provider 模式补 accept/reject Go 测试一对），check-acceptance-evidence.sh 全量 24 manifest 通过；mobile-sdk-design.md M0 段标注产出状态。**M0 验收①②③全绿，下一站 M1 Android SDK Alpha**。
- 附注（2026-09-22，覆盖率收口刀 + perf 对账根因修复）：**① 37cd598 CI（run 35692995018）failure 根因定位并修复**——TestPerfBaselineScriptWritesEvidence 的 ws-connections 场景「客户端 20 连接握手全成功、服务端 connected_clients 峰值恒 19」翻车；根因是 `HandleWebSocket` 用裸 `time.Now().UnixNano()` 生成连接 ID，CI 虚拟机时钟粒度粗时同批并发建连拿到相同纳秒值，hub `clients[client.ID]` 相互覆盖永久少计（fd5ba32 的峰值轮询只治登记滞后，治不了覆盖丢失；broadcast 慢客户端丢弃走 Send 缓冲 256 打不满，注册路径唯一"只减"分支就是 ID 覆盖）。修复提交 3643295：`newClientID()` = 纳秒时间戳 + 原子序号，同包并发唯一性测试 + 时钟退化测试，`TestPerfBaselineScriptWritesEvidence` 本地直跑 PASS。**② 100% 覆盖率收口**（用户指令：先扫缺口逐个补测，实在不可覆盖才登记 KNOWN_UNCOVERABLE）——严格逐包扫描出三处真实缺口，全部以测试补回 100%，**无一行需要 KNOWN_UNCOVERABLE 登记**：`suggestion/delivery` 91.7%（adapter `ExposureSummary` err 透传分支，未建表 sqlite 库直测）、`app/server` 99.8%（`BuildRuntime` TURN 装配层 gate 拒绝路径 + `wireRealtimeGateways` 校验失败分支，一个测试盖两块）、`quality/infra` 唯一残留未覆盖行 llm_score_provider.go:65（`TimeoutSeconds<=0` 零值 cancel 防御分支——构造器会归一为 30 导致经构造器的实例永远走不到，字面量构造零值超时直测）。`[no statements]` 包（analytics/contract·domain、automation/domain、conversation/domain、releasecheck）为零语句契约包无可覆盖；no-test-files 包（20 个 domain/contract 薄层）经 `-coverpkg` 全量交叉插桩验证语句全部被他包测试触达（合并 profile 唯一 0 计数块即 65 行，已修）。合并口径 run-tests.sh 100.0% + 逐包扫描全绿。37cd598 的 CI run 号因 failure 不入完成证据；本刀（3643295 + aa60f36）CI run 35714132203 绿，Script Checks（含 TestPerfBaselineScriptWritesEvidence 真实 perf 验收）全过，hub 修复与覆盖率收口均在真实 CI 验证；run 号回填提交 406c23b 的 CI run 35715559075 亦绿（回填提交 a89354b 的 CI run 35741271252 再绿，再后续 758f2cc 的 run 35743165953 亦绿——文档链到此收官，纯文档提交不再逐轮回填 run 号，完成证据锚定代码刀 3643295/aa60f36 → 35714132203）。

- 附注（2026-09-22）：**P2-0 RQ-5 收官，todo.md 可推进任务再度清零**——工作区待定建议批（用户 authored 的曝光/转化仓储与应用层 + 迁移 000010）由 38566cf 合入（CI 35685054335 绿），本刀补齐 WS 转化接线（realtime 窄接口零适配器 + handleTextMessage 同步归因 + 装配单实例共享）、SDK getInitialQuestions session_id 透传（demo-sdk bundle 同步）、验收脚本第 9 步（真实模式 node 原生 WS 发消息 + sqlite 直读断言曝光/转化，mock 模式 skipped 契约入 Go 测试），详见 P2-0 条目。同日固化刀（用户确认后执行）：ExposureSummary 管理面出口 + `/public/suggestions/` 三模板独立限流（含 paths 首匹配遮蔽顺序缺陷修复）。剩余未完成项仅 P1-1（真实 WeKnora/Dify 双路径运行证据，外部阻塞）。
...
- 附注（2026-09-21）：RAGFlow knowledge provider driver 立项落地（独立遗留项「知识库选型决策」①②两项随本刀执行，③新客户默认口径仍待产品）。七切片：①config 层 `RagFlowConfig`（Enabled/BaseURL/APIKey/DatasetID/Timeout/Search）+ 四模板加 ragflow 块（staging/production enabled:false）+ bootstrap 安全 gate（enabled 且 api_key/dataset_id 空 → 启动拒绝）；②`pkg/ragflow` HTTP client（envelope 双层判定 HTTP≥400 与业务 code!=0、retrieval 用 `knn_top_k`、multipart 上传 + POST chunks 触发异步解析、DELETE 带 JSON body、HealthCheck=GetDataset 存在性，100% 覆盖）；③`knowledgeprovider/ragflow` driver（删旧建新 upsert：按 `title.txt` 文件名查重——修复初版按裸标题查重导致永远查不中的去重失效缺陷；TopK/阈值 req→search→硬默认三档回退，100% 覆盖）；④configscope 租户/工作区作用域全量对齐（ResolveRagFlow + LoadRagFlowConfig + ScopedConfigDocument ragflow 段 + models 加 RagFlowJSON + 迁移 000009——列名 `rag_flow_json` 为 gorm NamingStrategy 对 RagFlowJSON 的推导结果，与 `we_knora_json` 先例一致，100% 覆盖）；⑤选择链 `selectKnowledgeSource` 插 ragflow 最高优先（健康检查失败 Warn 降级；仅 dify、weknora 均未启用且 requireHealthy 才立即失败），启动级 + scoped 请求级双接线，8 个新场景测试，app/server 100% 覆盖；⑥验收脚本 `test-ragflow-acceptance.sh`（mock 模式全自含：go build 真服务 + 内嵌 python3 RAGFlow mock 留痕 + sqlite 自起，/ai/status 断 provider=ragflow、检索命中、上传×2 删旧建新、sync、mock 留痕断言；real 模式拒私网）+ `make ragflow-acceptance` + ci.yml 验收白名单加 RagflowScript 两测试 + validate-acceptance-manifest ragflow case（正例测试 + 真实产物复验通过；修复脚本留痕 grep 单引号内变量不展开恒 false 缺陷）；⑦本文档 + LANDSCAPE 更新（§1 四 driver 表 + 选择链 ragflow→dify→weknora→pgvector、§3.1 已立项、§5/§6 路线与落地路径更新）。调试期间另修复 pkg/ragflow ListDocuments/GetDataset 查询串未 URL 转义（中文标题产生非法请求行）。全量门禁（gofmt/build/vet/boundaries/100% 逐包/security-acceptance/ragflow-acceptance/acceptance-evidence）本地全绿。提交 df86736 首跑 CI（35565315819）Integration 红——迁移水位断言三处硬编码（Makefile migrate-verify version、ci.yml 迁移验证、ci.yml 备份恢复对账）漏改，000009 落地后真实水位 9|f；d124dd5 修 ci.yml 两处、043a92f 补 Makefile 第三处，最终 CI 35566568357（HEAD 043a92f）全绿。教训已入 CI 硬约束清单：迁移 .up.sql 落盘同一刻即改全部断言。同日 P0-6/7/8 复核（CI 35615372455 绿）确认三项完成状态有效，仅 P0-8 有漏网残留（release.yml cache-dependency-path 与 CI_SELF_HOSTED.md 各一处已删目录 `apps/website-worker` 引用，release 触发时 setup-node 必失败）已清（0588c3b）。同日 RAGFlow 交付文档收尾刀：新建 docs/RAGFLOW_INTEGRATION.md（对齐 WEKNORA_INTEGRATION 形态：API 事实 v0.27.x 锚定/删旧建新 upsert 语义/调参回退链/四模板配置/mock+real 双模式验收/租户作用域/故障排除）+ 挂 vitepress 侧边栏（site-structure.ts 两处）+ acceptance-checklist §3 补 ragflow 行（mock 已验 real 待环境）与 G1-2 补脚本能力 + 过期口径同步（configuration-scopes 两处「dify 优先现状」改选择链、local-development 验收命令与链路描述、deployment 注释），docs build 复验绿。
...
- 附注（2026-09-20）：知识库选型调研文档产出（提交 47026ca，CI 35544351300 绿）——用户确认「Dify 不能严格算知识库，当初作为知识库插件定位不合适」后新增 docs/KNOWLEDGE_BASE_LANDSCAPE.md（RAGFlow 首选候选/MaxKB 次选/FastGPT 同定位问题/AnythingLLM 不推荐/Dify 定位降级为「编排平台 dataset 兼容」/WeKnora+RAGFlow 双路线建议/新 driver 落地路径）+ 挂侧边栏 + 修正 6 处「Dify 优先」文档口径（保留「当前代码选择链仍 dify 优先」现状事实，行为变更留待选型决策）+ 登记独立遗留项「知识库选型决策待产品确认」（RAGFlow 立项/选择链优先级调整/新客户默认口径）。同日 Dependabot PR #15 关闭（落后 main 的增量与工具链清理方向相反，swag 应以 tools.go 模式隔离不进 go.mod，excelize 部分已被 #16 覆盖）。同日单包覆盖率收口（提交 df53362，CI 35531002026 绿）——`go list ./internal/...` 逐包 `-cover` 扫描比全局合并 profile 更严格，补最后两个缺口（middleware AuditMiddlewareWithFailures 4xx 留痕直测 + analytics/infra avg-close Scan 错误分支），全部内部包单包 100.0%，「100%」检查口径升级为逐包扫描。同日 P2-4 第十二刀完成（deadcode 复查——刀 27/28 删除后新孤儿 15 处清除，提交 a512989，CI 35526544568 绿，净 -1011 行——errors Classify 全链/telemetry context·logfields/RequirePermissionsAll/getGrantedRoles/旧版 HealthHandler/NoopIndexJobRepository 等，复跑 deadcode 生产死链归零，余 34 项均为测试基建、多入口消费、接口契约或已登记产品黑洞，详见第十二刀条目；连带修正 docs/implementation 两处引用被删符号的漂移）。同日补登记独立遗留项（MetricsAggregator Snapshot 无读取出口，产品黑洞待产品/运维口径，提交 755b82c）。同日第十一刀完成（B 类半成品清单复核，提交 3090a27，CI 35510668953 绿，净 -1682 行——15 处死链清除 + 10 项翻案，B 类清单就此清零，详见第十一刀条目）。同日第十刀完成（RedisBus 订阅确认竞态修复，提交 822d788，CI 35499297519 绿——Publish 前置 waitUntilReady 门控 + 确认口径升级 pubsub.Receive 本连接确认替代 NUMSUB channel 级计数（多实例歧义消除），第九刀查实的独立遗留项就此闭环，详见第十刀条目）。同日 P0-1～P0-5 与 P1-1 标题标记对齐翻转（P0-1~P0-5 内部状态早已 `[x]` 且完成证据齐全，标题 `[!]` 为 2026-09-15 翻转 P0-6/7/8 时的遗漏；P1-1 对齐为 `[-]` 外部阻塞进行中），至此 todo.md 无 `[!]` 标记。同日第九刀完成（全仓死代码普查 A 类，提交 88d633a，净 -1107 行——deadcode 四入口可达性分析定位「收口刀孤儿」，B 类半成品清单另行登记，详见第九刀条目）。同日第八刀完成（assist 模块死链卫生扫描，提交 ac9ef2c，CI 35489519876 绿，净 -35 行——刀 24 方法论对单模块 41 个导出符号的首次全面应用，唯一死链 Repository.GetAnnotation 清除）。同日第七刀完成（FallbackPolicy/Matrix 死元数据链清除，提交 b0e4bc5，CI 35482372209 绿，净 -439 行，第六刀遗留登记的独立遗留项就此执行完毕）。同日第六刀完成（知识源选择门面，提交 99a42d2，CI 35479481212 绿）——启动/请求级双份选择链经 `selectKnowledgeSource` 合一，死存储 weKnoraClient/knowledgeBaseID 清除（构造 7 参缩 5 参），AIAssembly 收敛为统一视图五字段，净 -163 行。同日刀 22 验收脚本 flaky 根治（提交 1a0a96c，CI 35478211582 绿，gamification 验收 require_2xx 硬校验 + leaderboard 轮询 + 失败 dump）。另 P2-4 第五刀完成于 2026-09-19（known-gaps 清零，提交 871119c，CI 35476156786 绿）——`worker_job_duration_seconds` 经 app/worker 统一 `periodicJob` helper + 每轮 `TrackJob` 接线，10 个 worker 的重复 ticker 循环收敛为一处；`worker_jobs_total` 口径升级为 job 轮次（单轮业务失败计入 failure，WorkerJobFailures 告警真实生效）；Statistics/SLA 循环从 service 沉底改为 worker 侧驱动单轮方法（SLAMonitor 窄接口改单轮口径）；known-gaps.md 清零、dashboard 新增 duration p99 面板、runbook 三处同步。另 P1-6 已于 2026-09-19 收官（刀 21，提交 9d801a7，RunStandalone 唯一编排根）。至此仓库无未接线指标、无未完成可推进任务（剩余仅外部阻塞两项）
