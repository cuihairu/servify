...
migrate-verify:
	...
	if [ "$$version" != "6" ] || [ "$$dirty" != "f" ] || [ "$$tables" != "45" ]; then \
		echo "migrate-verify FAILED: expected version=6 dirty=f tables=45"; exit 1; \
	fi
	@echo "migrate-verify passed: version=6 dirty=false, 45 tables created, second run was a no-op"
