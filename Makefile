...
migrate-verify:
	...
	if [ "$$version" != "4" ] || [ "$$dirty" != "f" ] || [ "$$tables" != "41" ]; then \
		echo "migrate-verify FAILED: expected version=4 dirty=f tables=41"; exit 1; \
	fi
	@echo "migrate-verify passed: version=4 dirty=false, 41 tables created, second run was a no-op"
