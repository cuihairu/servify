...
migrate-verify:
	...
	if [ "$$version" != "3" ] || [ "$$dirty" != "f" ] || [ "$$tables" != "40" ]; then \
		echo "migrate-verify FAILED: expected version=3 dirty=f tables=40"; exit 1; \
	fi
	@echo "migrate-verify passed: version=3 dirty=false, 40 tables created, second run was a no-op"
