...
migrate-verify:
	...
	if [ "$$version" != "5" ] || [ "$$dirty" != "f" ] || [ "$$tables" != "43" ]; then \
		echo "migrate-verify FAILED: expected version=5 dirty=f tables=43"; exit 1; \
	fi
	@echo "migrate-verify passed: version=5 dirty=false, 43 tables created, second run was a no-op"
