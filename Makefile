...
migrate-verify:
	...
	if [ "$$version" != "15" ] || [ "$$dirty" != "f" ] || [ "$$tables" != "50" ]; then \
		echo "migrate-verify FAILED: expected version=15 dirty=f tables=50"; exit 1; \
	fi
	@echo "migrate-verify passed: version=15 dirty=false, 50 tables created, second run was a no-op"
