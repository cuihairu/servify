...
migrate-verify:
	...
	if [ "$$version" != "16" ] || [ "$$dirty" != "f" ] || [ "$$tables" != "50" ]; then \
		echo "migrate-verify FAILED: expected version=16 dirty=f tables=50"; exit 1; \
	fi
	@echo "migrate-verify passed: version=16 dirty=false, 50 tables created, second run was a no-op"
