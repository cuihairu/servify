...
migrate-verify:
	...
	if [ "$$version" != "7" ] || [ "$$dirty" != "f" ] || [ "$$tables" != "46" ]; then \
		echo "migrate-verify FAILED: expected version=7 dirty=f tables=46"; exit 1; \
	fi
	@echo "migrate-verify passed: version=7 dirty=false, 46 tables created, second run was a no-op"
