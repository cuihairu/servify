...
- Unread count (§4.3, mirroring the mobile `unreadCount`): while the session page is hidden (`markSessionHidden()`), arriving agent/AI content (WebSocket frames and reconciled history alike) increments `unreadCount` (broadcast via `unread-change`); customer echoes, system notices and streaming deltas never count (the final frame does). `markSessionVisible()` clears it — host wiring mirrors the mobile panel visibility
