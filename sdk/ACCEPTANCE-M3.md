...
§10 #5 下发侧真实凭证联调（FCM 服务账号 / APNs .p8，P1-1；注册端点刀 5 + 下发编排与 HTTP 传输刀 6 均已落地）、#2（guest token 签发——已随刀 8 落地，见上）。#1 已随刀 7 落地（`GET /api/v1/sessions/:session_id/messages`，消息 ID 单调游标，SDK 双端接入待联调）、#3 已随刀 9 落地（`POST .../read` + `GET .../unread`，服务端游标为 D7 客户端推导的增强面，SDK V1 仍以客户端推导为基线）、#4 已随刀 2 落地（见验收②）。§10 清单至此 #1—#4/#6 全部落地，仅剩 #5 真实凭证联调（P1-1）。
