...
func upsertPref(t *testing.T, repo *GormPreferenceRepository, ctx context.Context, sessionID, lang string) {
	t.Helper()
	...
	if err != nil {
		t.Fatalf("UpsertPreference(%s): %v", sessionID, err)
	}
}

// TestPreferenceUpsertCreateThenUpdate 同会话二次 upsert 走原行更新（保
// id/created_at，不新插行）。
func TestPreferenceUpsertCreateThenUpdate(t *testing.T) {
	...
	upsertPref(t, repo, ctx, "conv-1", "en")
	first, err := repo.GetPreference(ctx, "conv-1")
	if err != nil || first == nil {
		t.Fatalf("first get: %v %+v", err, first)
	}

	upsertPref(t, repo, ctx, "conv-1", "fr")
	second, err := repo.GetPreference(ctx, "conv-1")
...
func TestPreferenceScopeIsolation(t *testing.T) {
	...
	upsertPref(t, repo, owner, "conv-shared", "en")

	t.Run("cross tenant read is not found", func(t *testing.T) {
		got, err := repo.GetPreference(scopedCtx("tenant-b", "ws-1"), "conv-shared")
		if err != nil || got != nil {
	...
	t.Run("cross tenant upsert conflicts without touching owner row", func(t *testing.T) {
		...
		owned, err := repo.GetPreference(owner, "conv-shared")
	...
	t.Run("cross tenant delete does not remove owner row", func(t *testing.T) {
		if err := repo.DeletePreference(scopedCtx("tenant-b", "ws-1"), "conv-shared"); err != nil {
	...
	t.Run("empty scope sees all rows (local dev semantics)", func(t *testing.T) {
		got, err := repo.GetPreference(context.Background(), "conv-shared")
		if err != nil || got == nil {
...
func TestPreferenceDeleteIdempotent(t *testing.T) {
	...
	upsertPref(t, repo, ctx, "conv-del", "en")
	if err := repo.DeletePreference(ctx, "conv-del"); err != nil {
		t.Fatalf("delete: %v", err)
	}
	if err := repo.DeletePreference(ctx, "conv-del"); err != nil {
		t.Fatalf("second delete must stay successful, got %v", err)
	}
	got, err := repo.GetPreference(ctx, "conv-del")
	if err != nil || got != nil {
...
func TestPreferenceUpsertErrorAndRacePaths(t *testing.T) {
	...
	t.Run("raced row with different lang is overwritten", func(t *testing.T) {
		db := newPrefUnitTestDB(t)
		// 模拟并发：Create 落库后、复查前，同会话行被改写为他语言。
		if err := db.Callback().Create().After("gorm:create").Register("simul raced write", func(tx *gorm.DB) {
			tx.Exec("UPDATE translation_language_preferences SET target_lang = 'zz' WHERE conversation_session_id = 'conv-race'")
		}); err != nil {
			t.Fatalf("register callback: %v", err)
		}
		repo := NewGormPreferenceRepository(db)
		...
		got, err := repo.GetPreference(ctx, "conv-race")
		if err != nil || got == nil {
...
func TestPreferenceGetPassesThroughRawErrors(t *testing.T) {
	...
	repo := NewGormPreferenceRepository(db)
	_, err := repo.GetPreference(context.Background(), "conv-1")
