...
| `RemoteAssistSession` / `RemoteAssistAnnotation` | 33~46 | 模块自有（assist，**已迁入** `modules/assist/domain/models.go`，package 声明用唯一的 `assistdomain`，legacy 侧 `models.RemoteAssistX` 为别名） |
| `VoiceCall` | 25 | 模块自有（voice，**已迁入** `modules/voice/infra/models.go`，legacy 侧 `models.VoiceCall` 为别名） |
| `KnowledgeDoc` | 32 | 模块自有（knowledge，**已迁入** `modules/knowledge/domain/models.go`，含 `KnowledgeIndexJob` 与向量列值对象 `Embedding`，legacy 侧 `models.KnowledgeDoc` 等为别名，`NewEmbedding` 为转发函数） |
