...
4. **pgvector 保持默认轻量路径**：零外部依赖场景的默认选择不变。新客户交付口径已拍板（2026-09-21）：**pgvector 起步（零外部配置，交付首周跑通全链路），客户有知识库运营诉求时按解析深度选 WeKnora（轻量中文）或 RAGFlow（重解析）增强**，口径详见 deployment.md「新客户交付默认知识库口径」
