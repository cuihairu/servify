...
- 状态（2026-09-23）：**代码面完成（双端；验收① 注册链路已通（刀 5），推送端到端仍待下发/凭证配套，如实标注，不假装可验收；逐条锚定见 `sdk/ACCEPTANCE-M3.md`）**。刀序列：刀 1 Branding offlineText（4fd2ead）→ 刀 2 服务端访客工单端点（48813a6，§10 #4；迁移水位断言随 000011 递增 337f613/9a36216）→ 刀 3a createTicket 门面 + TicketSummary 摘要（7b6ced4）→ 刀 3b 双端 UI 入口（11ae3df，iOS 15 兼容 + 测试时序修正 8d928de）→ 刀 4 pushTokenProvider 注册口 + CocoaPods 评估（5a81558）→ 刀 5 推送注册链路（服务端 §10 #5 注册端点 + SDK 双端真实上报）。① 注册链路已通：`pushTokenProvider` 未配置→unsupported / provider 返 nil→静默（宿主未授权是正常态）/ 有 token→真实上报 `POST /api/v1/push/register`（2xx→true；IO/HTTP 失败→network+false 可重报；刀 5 双端 6 用例镜像；服务端迁移 000012_push_tokens，幂等键 session_id+platform，token 不回显，三包测试 orchestration 5 + delivery 7 + handlers 5）；端到端链路（后台→系统→点开→增量补拉→未读归零）仍缺 §10 #5 推送下发（sender，FCM/APNs 凭证 P1-1 阻塞联调）+ §10 #1 游标端点，未通；offlineText 断线提示双端锚定（握手失败/重连耗尽/销毁不提示/不计未读，ConnectionLifecycleTest ↔ ConnectionLifecycleTests 用例名逐一对应）。② 双端闭环：门面 createTicket 自动组 ai_summary（TicketSummary 最近 10 条，[访客]/[AI]/[客服] 标注，System 提示行跳过，空则不带键）+ UI 入口（标题必填/描述选填/提交中禁用/失败重试提示/成功 appendSystemHint 进 history 不计未读）+ 服务端三包测试（orchestration 5 + delivery 3 + handlers 5）；坐席侧可见走管理面既有 tickets 列表/详情（同表同响应链，ai_summary 进 ticketResponse），无新开发面。③ 体积门禁持续绿：Android R8 口径 + iOS 分发 zip 口径 CI 独立 step 每次 push 复核，M3 期间无漂移。CocoaPods 兼容层评估：`docs/mobile-sdk-cocoapods-evaluation.md`——零已知 CocoaPods-only 接入方，SPM-only 单轨维持，按需重建触发条件已记录，复核时点 M4 文档站定稿。真机手工项（推送/前后台/弱网）随验收①外部配套一并待执行。
...
| 5 | 推送 token 注册 + 会话消息推送下发 | M3 | 注册链路 ✅ 已落地（刀 5）：`POST /api/v1/push/register` 免认证（体 `{session_id, platform, token}`，platform 白名单 ios/android），幂等键 (session_id, platform) 重复注册保活刷新、换平台独立行，token 落库不回显（迁移 000012）；SDK 双端 `registerPushToken()` 真实上报（2xx→true，失败 network+false）。下发 ⏳ 未实现（sender + 凭证 P1-1 阻塞联调） |
